-- Exclusivo do PDF Casa de Banho. Não aplicar na base do Kit Estudante.
CREATE TABLE IF NOT EXISTS kit_events (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  occurred_at TEXT NOT NULL,
  lead_id INTEGER,
  source TEXT NOT NULL CHECK(source = 'pdf-casa-de-banho'),
  event TEXT NOT NULL,
  field_name TEXT,
  field_value TEXT,
...
CREATE INDEX IF NOT EXISTS idx_banho_events_request ON kit_events(request_id, event, status);
CREATE INDEX IF NOT EXISTS idx_banho_events_time ON kit_events(occurred_at);
...
WITH pedidos AS (
  ...
  FROM kit_events WHERE event = 'banho_pdf_sent' AND status = 'success'
  GROUP BY request_id
), ultimos_meta AS (
  SELECT request_id, status, error_code,
    ROW_NUMBER() OVER (PARTITION BY request_id ORDER BY occurred_at DESC, id DESC) AS posicao
  FROM kit_events WHERE event = 'banho_meta_registration'
), aceites AS (
  SELECT DISTINCT request_id FROM kit_events
  WHERE event = 'banho_meta_registration' AND status = 'success'
)
