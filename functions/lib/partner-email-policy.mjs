...
export async function checkPartnerEmailPolicy(env,email,eventId){
 return (await resolvePartnerEmailPolicy(env,email,eventId)).allowed;
}
export async function resolvePartnerEmailPolicy(env,email,eventId,refreshLink=false){
 ...
 const response=await fetch(url,{method:'POST',headers:{Authorization:`Bearer ${env.CLEANING_DASHBOARD_API_TOKEN}`,'Content-Type':'application/json'},body:JSON.stringify({email,event_id:eventId,refresh_link:refreshLink}),signal:AbortSignal.timeout(8000)});
