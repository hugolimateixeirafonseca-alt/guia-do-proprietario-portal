...
for(const existing of [false,true])test('application registers correct partner group, existing='+existing,async()=>{const original=globalThis.fetch;const calls=[];globalThis.fetch=async(url,init={})=>{calls.push({url:String(url),body:init.body?JSON.parse(init.body):null});return new Response('{}',{status:!init.method&&!existing?404:200});};try{const r=await onRequestPost({request:request({...payload,event_id:'application:12345678-1234-1234-1234-123456789abc'}),env:{MAKE_PARTNER_NOTIFICATIONS_SECRET:secret,SENDER_API_TOKEN:'test'}});assert.equal(r.status,200);const group=calls.find(c=>c.url.endsWith('/groups/aOoGvG'));assert.deepEqual(group.body.subscribers,[payload.partner_email]);assert.equal(group.body.trigger_automation,false);assert.equal(calls.some(c=>c.url.includes('egK8WG')||c.url.includes('bWv1LJ')),false);if(!existing)assert.deepEqual(calls.find(c=>c.url.endsWith('/subscribers')).body.groups,['aOoGvG']);}finally{globalThis.fetch=original;}});
test('failed partner group sync does not report success or send confirmation email',async()=>{const original=globalThis.fetch;let sent=false;globalThis.fetch=async(url)=>{if(String(url).endsWith('/message/send'))sent=true;return new Response('{}',{status:503});};try{const r=await onRequestPost({request:request({...payload,event_id:'application:12345678-1234-1234-1234-123456789abc'}),env:{MAKE_PARTNER_NOTIFICATIONS_SECRET:secret,SENDER_API_TOKEN:'test'}});assert.equal(r.status,502);assert.equal(sent,false);}finally{globalThis.fetch=original;}});
