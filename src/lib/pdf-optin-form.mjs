    ...
    try {
      const response = await win.fetch(endpoint, {
        ...
        body: JSON.stringify({
          ...
          consentVersion: form.dataset.consentVersion, source,
      ...
      if (!response.ok || result.ok !== true) {
        const messages = {
          ...
          invalid_consent: 'Autorize o envio do PDF para receber o guia.',
          too_many_requests: 'Já foram feitos vários pedidos. Aguarde alguns minutos antes de tentar novamente.'
        };
        throw new Error(messages[result.error] || 'Não foi possível enviar o guia agora. Tente novamente dentro de momentos.');
      }
      trackKitRegistration(doc, win, result.metaEventId, source);
      doc.getElementById('kj-success-email').textContent = email.value.trim();
      form.hidden = true; success.hidden = false; success.focus();
      win.location.assign(thankYouUrl(win.location.href));
    } catch (error) {
