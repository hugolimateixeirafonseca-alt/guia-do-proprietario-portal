...
export const GET: APIRoute = async ({ site }) => {
  ...
  const items = artigos.map((artigo) => { const url = new URL(`/${artigo.data.pilar}/${artigo.id}/`, base).toString(); return `<item><title>${escapeXml(artigo.data.titulo)}</title><link>${url}</link><guid>${url}</guid><description>${escapeXml(artigo.data.descricao)}</description><pubDate>${artigo.data.publicado_em.toUTCString()}</pubDate></item>`; }).join("");
